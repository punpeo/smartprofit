...
// FillOrderCost 通过 SKU JOIN products 表自动计算：FillOrderTotalNum × product.default_fill_order_cost
type DailyFillOrderLog struct {
	...
	FillOrderTotalNum int     `db:"fill_order_total_num" json:"fill_order_total_num"` // 补单总订单数
	...
	FillOrderCost     float64 `db:"fill_order_cost" json:"fill_order_cost"`           // 补单商品总成本 = FillOrderTotalNum × product.default_fill_order_cost（自动计算）
